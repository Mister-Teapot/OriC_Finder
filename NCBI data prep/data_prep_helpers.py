...
if __name__ == '__main__':
    ...
    fieldnames = ['RefSeq', 'Sequence_length', 'GC_Concentration']
    fieldnames.extend([f'oriC_middles_{i}' for i in range(max_oriCs)])
    fieldnames.extend([f'Avg_Occurance_oriC_{i}' for i in range(max_oriCs)])
    fieldnames.extend([f'Z_Occurance_oriC_{i}' for i in range(max_oriCs)])
    fieldnames.extend([f'G_Occurance_oriC_{i}' for i in range(max_oriCs)])
    fieldnames.extend([f'D_Occurance_oriC_{i}' for i in range(max_oriCs)])
    fieldnames.extend([f'Prediction_oriC_{i}' for i in range(max_oriCs)])

    merge_csvs('../exp_csvs/', '../exp_csvs/Predicted_exp.csv', fieldnames, length=1)
