...
def train_model(X, y, classifier, k):
    ...
    for train_index, vali_index in k_fold.split(X):
        ...
        accuracy_of_current_fold = accuracy_score(prediction , vali_labels)
...
def fine_tune(X, y, model_dict, parameter_dict, print_found_parameters=True):
    best_params_models = []
    for name, parameters in parameter_dict.items():
        model = model_dict[name]
        cv = KFold(n_splits=K)
        grid_search = GridSearchCV(model, parameters, cv=cv, n_jobs=2, verbose=4, scoring="accuracy", ).fit(X, y)
